...
app.post('/api/login', (req, res) => {
    ...
    db.query('SELECT * FROM utilisateurs WHERE email = ? AND password = ?', [email, password], (error, results) => {
...
app.put('/api/equipement/:id', (req, res) => {
    ...
    if (!status) {
        return res.status(400).json({ success: false, message: 'Nom et statut sont requis.' });
    }
    db.query('UPDATE equipement SET status = ? WHERE id = ?', [status, eqid], (error, results) => {
        if (error) return res.status(500).send(error);
...
app.get('/api/timeline/equipment/:id', (req, res) => {
    ...
    db.query(sql, [eqid], (err, result) => {
        ...
        if (result.length > 0) {
            ...
            return res.status(404).json({ Message: "Aucune timeline trouvée pour cet équipement" });
        }
    });
});

// Route pour récupérer le statut d'un équipement
...
app.post('/api/equipement', (req, res) => {
    const { name, type, serialNumber, model, vendor, purchaseDate, warrantyEnd, currentAssignment } = req.body;

    // Assurez-vous que tous les champs nécessaires sont fournis
    if (!name || !type || !serialNumber || !model || !vendor || !purchaseDate || !warrantyEnd) {
        return res.status(400).json({ success: false, message: 'Tous les champs sont requis.' });
    }

    const sql = `INSERT INTO equipement (name, type, serialNumber, model, vendor, purchaseDate, warrantyEnd, currentAssignment) 
                 VALUES (?, ?, ?, ?, ?, ?, ?, ?)`;

    db.query(sql, [name, type, serialNumber, model, vendor, purchaseDate, warrantyEnd, JSON.stringify(currentAssignment)], (err, results) => {
        if (err) {
            return res.status(500).json({ Message: "Erreur dans le serveur" });
        }
        res.status(201).json({ success: true, message: 'Équipement ajouté avec succès.', id: results.insertId });
    });
});
...
app.delete('/api/equipement/:id', (req, res) => {
    ...
    db.query(sql, [eqid], (err, results) => {
        ...
        if (results.affectedRows > 0) {
            return res.json({ success: true, message: 'Équipement supprimé avec succès.' });
        } else {
...
app.post('/api/timeline', (req, res) => {
    const { equipmentId, date, type, title, description, actor, location, icon } = req.body;

    // Vérifiez que tous les champs nécessaires sont fournis
    if (!equipmentId || !date || !type || !title || !description || !actor || !location || !icon) {
        return res.status(400).json({ success: false, message: 'Tous les champs sont requis.' });
    }
    ...
    db.query(sql, [equipmentId, date, type, title, description, actor, location, icon], (err, results) => {
        if (err) {
            return res.status(500).json({ Message: "Erreur dans le serveur" });
        }
        res.status(201).json({ success: true, message: 'Entrée ajoutée à la timeline avec succès.', id: results.insertId });
    });
});
});
